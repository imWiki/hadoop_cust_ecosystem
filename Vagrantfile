...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    config.vm.provider "docker" do |d|
	    d.image = "vignesh/docker-vagrant-ubuntu:xenial"
	    d.has_ssh = true
	    d.ports = ["8088:8088","8080:8080","9083:9083","4040:4040","8888:8888","16010:16010","9999:9999"]
    end
    ...
    config.vm.define "vigneshm" do |node|
        node.vm.network :private_network, ip: '10.211.55.101'
        node.vm.hostname = 'vigneshm'
	...
	# Optional components - uncomment to include
        ...
        node.vm.provision :shell, path: 'scripts/data_proc.sh', run: 'always'
    end
end
